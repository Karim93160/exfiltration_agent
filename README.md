 ...
 * Ouvrez Termux sur votre appareil Android.
 * Clonez le dépôt GitHub :
   git clone [VOTRE_URL_DU_DÉPÔT_GITHUB] exfiltration_agent

   (N'oubliez pas de remplacer [VOTRE_URL_DU_DÉPÔT_GITHUB] par l'URL réelle de votre dépôt !)
 * Lancez le script d'installation tout-en-un :
   Naviguez dans le répertoire de l'agent et exécutez le script d'installation. Ce script est conçu pour tout gérer : installation des dépendances système et Python, et lancement du panneau de contrôle web.
   cd exfiltration_agent
chmod +x setup_termux.sh exf_agent.py control_panel.py # Assure les permissions d'exécution
./setup_termux.sh
 ...
 * Surveillez l'Activité et Gérez l'Agent :
 ...
 * Téléchargez les Logs pour Analyse :
...
└── backup_agent.sh           # Script pour créer une sauvegarde du projet

# --- Fichiers à ignorer par Git (via .gitignore) ---
# .gitignore                  # Fichier listant les éléments à ne PAS inclure sur GitHub
# shared_config.json          # Contient la clé AES et les paramètres, généré localement
# agent_logs.enc              # Logs chiffrés générés par l'agent
# agent_logs.enc.<num>        # Backups des logs
# control_panel.log           # Logs du panneau de contrôle
# *.pyc, __pycache__/ etc.    # Fichiers compilés/caches Python
# test_scan_dir/, etc.        # Répertoires de test locaux
