...
class Logger:
    ...
    def _write_log(self, level: int, message: str):
        ...
        if self.cipher:
            try:
                data_to_write = self.cipher.encrypt(formatted_message_bytes)
            except Exception as e:
    ...
    def _rotate_logs(self):
        self._write_log(self.INFO, "Rotation du fichier de log...")
        ...
        self._write_log(self.INFO, "Rotation des logs terminée.")
            ...
            try:
                with open(f_path, 'rb') as f:
                    for line_bytes in f:
                        ...
                        if self.cipher:
                            try:
                                decrypted_data_bytes = self.cipher.decrypt(line_bytes)
                                # --- AJOUT DE DÉBOGAGE ICI ---
                                decoded_json_string = decrypted_data_bytes.decode('utf-8')
                                # print(f"[DEBUG] JSON déchiffré avant parsing : '{decoded_json_string}'")
                                # Si l'erreur se produit à nouveau, affiche cette ligne pour voir le contenu exact
                                # qui pose problème à json.loads()
                                # -----------------------------
                                entry = json.loads(decoded_json_string)
                                decrypted_entries.append(entry)
                            except (ValueError, KeyError, json.JSONDecodeError) as e:
                                print(f"[ERROR] [Logger] Erreur lors du déchiffrement ou parsing JSON d'une ligne dans '{f_path}': {e}. Ligne (partielle): {line_bytes[:100]}...")
                                # --- AJOUT DE DÉBOGAGE POUR L'ERREUR JSON ---
                                if 'decoded_json_string' in locals():
                                    print(f"[DEBUG] Contenu déchiffré qui a échoué au parsing JSON : '{decoded_json_string}'")
                                # --------------------------------------------
                            except Exception as e:
                                print(f"[ERROR] [Logger] Erreur inattendue lors du déchiffrement d'une ligne dans '{f_path}': {e}. Ligne (partielle): {line_bytes[:100]}...")
                        else:
                            try:
                                entry = json.loads(line_bytes.decode('utf-8', errors='ignore'))
                                decrypted_entries.append(entry)
                            except json.JSONDecodeError as e:
                                print(f"[ERROR] [Logger] Erreur lors du parsing d'une ligne de log en clair dans '{f_path}': {e}. Ligne (partielle): {line_bytes[:50]}...")
...
if __name__ == "__main__":
    ...
    num_entries_to_fill = int(Logger.MAX_LOG_SIZE / len(logger_rotate._format_message(Logger.INFO, "a")) * 1.5)
